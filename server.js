...
// PUT Image (UPDATE)
app.put('/api/livewell/:_id', upload.single('photo'), function(req, res) {
    Livewell.findById(req.params._id, function(err, livewell) {
...
// DELETE Id
app.delete('/api/livewell/:_id', function(req, res) {
    Livewell.remove({ _id: req.params._id}, function(err, output) {
        if (err) return res.status(500).json({ error: "database failure" });
